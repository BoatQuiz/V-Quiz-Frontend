...
export default function TestSubmitAnswer() {
  ...
  async function getNextQuestion() {
    ...
    setSession({
      id: result.Data?.Session.SessionId || "",
      score: result.Data?.Session.Score || 0,
      numUsedQuestions: result.Data?.Session.NumUsedQuestions || 0,
    });
  }
  ...
  return (
    <div className="flex gap-2">
      ...
      <button className="border p-2 " onClick={testSubmit}>
        Skicka svaret
      </button>
      <button className="border p-2 " onClick={getNextQuestion}>
        Hämta nästa fråga
      </button>
      <button className="border p-2 " onClick={flagQuestion}>
        Klicka mig för felrapport
      </button>
    </div>
  );
}
